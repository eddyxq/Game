...
func animation_loop():
	if anim_finished:
		...
		if velocity.x == 0 && (abs(position.x - player.get_global_position().x) < 135) && (abs(position.y - player.get_global_position().y) < 135)  && health > -1:
...
# ranged attack that shoots a projectile
func shooting_attack():
	var projectile = preload("res://scenes/enemy/WispProjectile.tscn").instance()
	get_parent().add_child(projectile)
	projectile.position = $PositionCenter.global_position
	projectile.set_projectile_direction(direction_facing)
