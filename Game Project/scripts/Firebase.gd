...
###############################################################################
# server and databse logic that utilizes Google Firebase
###############################################################################

const API_KEY := "AIzaSyB5sbUPYOdCOKaJNw0JOp4Dvfe3CU0LeNk"
const PROJECT_ID := "blaze-f99dd"
...
func register(email: String, password: String, http: HTTPRequest) -> void:
	...
	var _request = http.request(REGISTER_URL, [], false, HTTPClient.METHOD_POST, to_json(body))
...
func login(email: String, password: String, http: HTTPRequest) -> void:
	...
	var _request = http.request(LOGIN_URL, [], false, HTTPClient.METHOD_POST, to_json(body))
	var result := yield(http, "request_completed") as Array
	if result[1] == 200:
		user_info = _get_user_info(result)
...
func save_document(path: String, fields: Dictionary, http: HTTPRequest) -> void:
	...
	var _request = http.request(url, _get_request_headers(), false, HTTPClient.METHOD_POST, body)


func get_document(path: String, http: HTTPRequest) -> void:
	var url := FIRESTORE_URL + path
	var _request = http.request(url, _get_request_headers(), false, HTTPClient.METHOD_GET)
...
func update_document(path: String, fields: Dictionary, http: HTTPRequest) -> void:
	...
	var body := to_json(document)
	var url := FIRESTORE_URL + path
	var _request = http.request(url, _get_request_headers(), false, HTTPClient.METHOD_PATCH, body)


func delete_document(path: String, http: HTTPRequest) -> void:
	var url := FIRESTORE_URL + path
	var _request = http.request(url, _get_request_headers(), false, HTTPClient.METHOD_DELETE)
